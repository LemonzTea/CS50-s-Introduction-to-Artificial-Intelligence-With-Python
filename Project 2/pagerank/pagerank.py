...
def iterate_pagerank(corpus, damping_factor):
    ...
    for page, links in corpus.items():
        current_ranking[page] = 1 / float(len(corpus))
    # print(sum(current_ranking.values()))
    ...
    while repeat:
        ...
        for page, links in corpus.items():            
            # Calculate 2nd part of the score
            sum_score = 0
            for page2, links2 in corpus.items():
                if page in links2:
                    sum_score += round((current_ranking[page2] / float(len(links2))), 4)
                elif len(links2) == 0:
                    sum_score += round((current_ranking[page2] / float(len(corpus))), 4)
            ranking[page] = ((1 - damping_factor) / (len(corpus))) + (damping_factor * sum_score)

        # check current and previous score difference 
        # print(f"{current_ranking} + Total Probability:{sum(current_ranking.values())}")
        for page, score in current_ranking.items():
            if abs(score - ranking[page]) >= 0.001:
                repeat = True
                break
        
        current_ranking = ranking.copy()
